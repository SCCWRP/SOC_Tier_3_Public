...
ranges <- read.csv("/Users/katieirving/SCCWRP/SOC WQIP - Flow Ecology Study - General/Tier3_analysis/Willow_Flow_Ranges_Ruleset.csv")
head(ranges)
...
f <- f[c(1,6,9)] ## take only existing conditions
f
...
## save
write.csv(time_statsx, paste0(outdir,"Willow/Current/02_Willow_Current.csv"))

# Water Conservation ------------------------------------------------------------

f <- list.files() ## list scenario files
f <- f[c(5,8,11)] ## take only water cons conditions
f
## empty DF for time stats
time_statsx <- NULL
...
time_statsx <- na.omit( time_statsx) ## remove missing subbasins
## 
write.csv(time_statsx, paste0(outdir,"Willow/Water_Conservation/02_Willow_WaterCons.csv"))


# Reference ---------------------------------------------------------------

f <- list.files() ## list scenario files
f <- f[c(2, 7, 10)] ## take only water cons conditions
f
## empty DF for time stats
time_statsx <- NULL
...
time_statsx <- na.omit( time_statsx) ## remove missing subbasins
## 
write.csv(time_statsx, paste0(outdir,"Willow/Reference/02_Willow_Reference.csv"))
